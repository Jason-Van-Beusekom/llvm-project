...
void Flang::AddNVPTXTargetArgs(const ArgList &Args,
  ...
  // we cannot use addClangTargetOptions, as it appends unsupported args for flang:
  // -fcuda-is-device, -fno-threadsafe-statics, -fcuda-allow-variadic-functions and 
  // -target-sdk-version
  // Instead we manually detect the CUDA installation and link libdevice
  const ToolChain &TC = getToolChain();
  const Driver &D = TC.getDriver();
